...
def run_prac(win, thisExp, fixation_text, warning_image_1, target_image, trialClock, rt_list, acc_list, results_dir, resultFile_name, used_keyList, correct_responses):
    ...
    for index, row in trials.iterrows():
        ...
        win.flip()
        thisExp.addData('cue_onsetTime', trialClock.getTime())
        core.wait(0.1)  # Assume some duration for the warning
        fixation_text.setAutoDraw(False)
        # Middle fixation
        fixation_text.setAutoDraw(True)
        win.flip()
        ...
        # Fixation end
        if reaction_time is not None:
            setAttrib(trialAttributes, "DurationOfFixationEnd",
                      getAttrib(trialAttributes, "DurationOfFixationEnd") - rt * 1000 - 40)
        else:
            setAttrib(trialAttributes, "DurationOfFixationEnd",
                      getAttrib(trialAttributes, "DurationOfFixationEnd") - getAttrib(trialAttributes,
                                                                                      "DurationOfTarget") - 40)
...
def run_behav(win, thisExp, fixation_text, warning_image_1, target_image, trialClock, rt_list, acc_list, results_dir, resultFile_name, used_keyList, correct_responses):
    ...
    for index, row in trials.iterrows():
        ...
        # Fixation end
        if reaction_time is not None:
            setAttrib(trialAttributes, "DurationOfFixationEnd", getAttrib(trialAttributes, "DurationOfFixationEnd") - rt * 1000 - 40)
        else:
            setAttrib(trialAttributes, "DurationOfFixationEnd", getAttrib(trialAttributes, "DurationOfFixationEnd") - getAttrib(trialAttributes, "DurationOfTarget") - 40)
